'''
Code imported from assgn6 cp_hw6.py, credits to Ioannis Gkioulekas
'''
...
def calibrateIntrinsic(images, checkerboard, dW):    
...
X_CAPT = np.float32([])
Y_CAPT = np.float32([])
def calibrateExtrinsic(img_path, mtx, dist, dX, dY):
    ...
    def capture_click(event, x_click, y_click, flags, params):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            cv2.drawMarker(color_img, (X_CAPT[-1], Y_CAPT[-1]), (0, 0, 255), cv2.MARKER_TILTED_CROSS, 30)
    ...
    #cv2.line((axis_img[0, 0, 0], axis_img[1, 0, 0]), (axis_img[0, 0, 1], axis_img[1, 0, 1]), 'r-', linewidth=2)
    #plt.plot((axis_img[0, 0, 0], axis_img[2, 0, 0]), (axis_img[0, 0, 1], axis_img[2, 0, 1]), 'g-', linewidth=2)
    #plt.plot((axis_img[0, 0, 0], axis_img[3, 0, 0]), (axis_img[0, 0, 1], axis_img[3, 0, 1]), 'b-', linewidth=2)
